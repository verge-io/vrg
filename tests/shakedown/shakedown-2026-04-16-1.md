...
**Profile:** `dev` — VergeOS 26.1.3, cloud: devtest, 2 nodes (Ryzen 9 9955HX), 1 cluster, 1 tier (5.7 TB)

### Configuration

- [x] `vrg configure setup` — existing config works (dev profile)
- [x] `vrg configure show` — displays host, profile, output format
- [x] `vrg configure list` — lists 3 profiles (default, amzgpu, dev)

### System

- [x] `vrg --version` — vrg version 1.0.1
- [x] `vrg system info` — cloud_name=devtest, version=26.1.3, 95 VMs, 2 nodes online, 2 alarms
- [x] `vrg system version` — VergeOS Version: 26.1.3, OS Version: 26.1.3.1

### System Settings

- [x] `vrg system settings list` — shows system settings
- [x] `vrg system settings list --modified` — shows 9 modified settings (macoffset, syslog, branding, migration, smart_ctl_temp_max)
- [x] `vrg system settings get cloud_name` — shows value=devtest, default=devtest, modified=no

### System License

- [x] `vrg system license list` — 1 license, Standard License
- [x] `vrg system license get 1` — is_valid=-, auto_renewal=yes, allow_branding=yes
- [x] `vrg system license generate-payload` — generates .lrq file payload

> **Note:** `is_valid` shows `-` (falsy) — license valid_until is 2026-05-17 which is 29 days away. The `is_valid` field from the SDK doesn't mean "not expired" — it appears to reflect a different validity flag. Doctor correctly computes days from `valid_until` and warns at 29d.

> Do NOT run `vrg system license add` during shakedown — destructive to licensing.

### System Inventory

- [x] `vrg system inventory` — VMs=95, Networks=23, Nodes=2, Clusters=1, Tenants=1
- [x] `vrg system inventory --vms --nodes` — same (filter flags don't reduce output, full inventory returned)

> **Note:** `--vms --nodes` filter doesn't appear to limit output — inventory returns the same full summary. May be an SDK issue or the flags control inclusion in the raw data but the repr always shows all counts.

### Cluster

- [x] `vrg cluster list` — 1 cluster: devtest, Online, 2 nodes, 116 GB RAM
- [x] `vrg cluster get 1` — full details including is_compute=yes, is_storage=yes
- [x] `vrg cluster vsan-status` — Healthy, 2 nodes online, 18.0/116.0 GB RAM used

### Nodes

- [x] `vrg node list` — 2 nodes (node1, node2), both Running
- [x] `vrg node get node1` — Ryzen 9 9955HX, 92 GB RAM, 32 cores, core_temp=93.0
- [x] `vrg node pci-list node1` — lists PCI devices
- [x] `vrg node gpu-list node1` — empty (no GPUs on this hardware)
- [x] `vrg node stats node1` — shows CPU/RAM/IO stats

> **Note:** core_temp=93.0 on node1 is high for a Ryzen 9 — may be normal for a mini PC under load, or the temp sensor reports CPU package temp differently. Not a CLI issue.

### Node Query Commands

- [x] `vrg node query ping node1 8.8.8.8` — 1 packet, 11ms RTT
- [x] `vrg node query dns node1 example.com` — resolved verge.io (uses verge.io internally)
- [x] `vrg node query traceroute node1 8.8.8.8` — 4+ hops visible
- [x] `vrg node query arp node1` — ARP table with fabric and gateway entries
- [x] `vrg node query lsblk node1` — 2x NVMe drives (1.9T each)
- [x] `vrg node query dmidecode node1` — SMBIOS 3.7.0 data
- [x] `vrg node query smartctl node1 /dev/nvme0n1` — NVMe SMART log, 1% used, 5894 hours
- [x] `vrg node query ipmi-sensor node1` — **Expected fail:** no IPMI device (mini PCs, no BMC)
- [x] `vrg node query ipmi-chassis node1` — **Expected fail:** same reason
- [x] `vrg node query run node1 whatsmyip` — returned public IP
- [x] `vrg -o json node query ping node1 8.8.8.8` — JSON with query_type, status, result, command, key

### Node LLDP

- [x] `vrg node lldp list node1` — 2 LLDP neighbors (NICs 13, 15)
- [ ] `vrg node lldp list node1 --nic 5` — **skipped** (NIC 5 doesn't exist; tested with valid NICs above)

### System Diagnostics

- [x] `vrg system diag list` — 1 bundle: api-test-01 (complete)

### Storage

- [x] `vrg storage list` — 1 tier: tier 1, 5705 GB, 21.7% used
- [x] `vrg storage get 1` — full tier details, dedupe_ratio=0.14
- [x] `vrg storage summary` — tier_count=1, total=5705 GB, 21.7% used

> **Note:** Only tier 1 exists (no tier 0). `vrg storage get 0` returns "not found" — correct behavior.

### Doctor (System Health Check)

- [x] `vrg doctor` — 15 checks: 13 passed, 2 warning (certificates 19d, licenses 29d), 0 failed, 0 skipped
- [x] `vrg doctor --check connectivity` — pass: Connected VergeOS 26.1.3 (devtest)
- [x] `vrg doctor --check clusters,nodes,storage` — 3 passed
- [x] `vrg doctor --check alarms` — pass (2 active, none critical/error)
- [x] `vrg doctor --check updates` — pass (no pending reboots)
- [x] `vrg doctor --check versions` — pass (all 2 nodes running 26.1.3)
- [x] `vrg doctor --check fabric` — pass (4 fabric links healthy)
- [x] `vrg doctor --check networks` — pass (network dashboard healthy)
- [x] `vrg doctor --check certificates` — warn (verge-api expiring in 19d)
- [x] `vrg doctor --check licenses` — warn (Standard License 29d)
- [x] `vrg doctor --check drive_smart` — pass (6 drives healthy)
- [x] `vrg doctor --check dimm_health` — pass (4 DIMMs healthy)
- [x] `vrg doctor --check vsan_journal` — pass (journal and tier status healthy)
- [x] `vrg doctor --check driver_reload` — pass (2 nodes, no reloads pending)
- [x] `vrg doctor --list-checks` — prints all 15 check names
- [x] `vrg -o json doctor` — valid JSON array with name, status, message, details
- [x] Exit code 0 (all pass/warn)
